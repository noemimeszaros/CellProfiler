...
-  *{ENHANCE}:* Produce an image whose intensity is largely composed of
...
-  *{E_SPECKLES}:* A speckle is an area of enhanced intensity relative
   to its immediate neighborhood. The module enhances speckles using a
   white tophat filter, which is the image minus the morphological
   grayscale opening of the image. The opening operation first
   suppresses the speckles by applying a grayscale erosion to reduce
   everything within a given radius to the lowest value within that
   radius, then uses a grayscale dilation to restore objects larger than
   the radius to an approximation of their former shape. The white
...
-  *{E_NEURITES}:* Neurites are taken to be long, thin features of
   ...
   described below.
-  *{E_DARK_HOLES}:* The module uses morphological reconstruction (the
   rolling-ball algorithm) to identify dark holes within brighter areas,
   or brighter ring shapes. The image is inverted so that the dark holes
   turn into bright peaks. The image is successively eroded and the
   eroded image is reconstructed at each step, resulting in an image
   which is missing the peaks. Finally, the reconstructed image is
   ...
   performed.
-  *{E_CIRCLES}:* The module calculates the circular Hough transform of
   the image at the diameter given by the feature size. The Hough
   ...
   **IdentifySecondaryObjects** to find whole, circular objects using a
   watershed.
-  *{E_TEXTURE}:* **EnanceOrSuppressFeatures** produces an image whose
   intensity is the variance among nearby pixels. This method weights
   pixel contributions by distance using a Gaussian to calculate the
   weighting. You can use this method to separate foreground from
...
""".format(**{
        ...
        self.object_size = cellprofiler.setting.Integer(
            ...
            2,
            doc="""\
*(Used only if circles, speckles or neurites are selected, or if
...
""".format(**{
                "HELP_ON_MEASURING_DISTANCES": HELP_ON_MEASURING_DISTANCES
            })
        )
...
-  *{E_DIC}:* Specifies the amount of smoothing of the image in the
   ...
   the noise but smooths the resulting image. Increase the smoothing to
   eliminate streakiness and decrease the smoothing to sharpen the image.
